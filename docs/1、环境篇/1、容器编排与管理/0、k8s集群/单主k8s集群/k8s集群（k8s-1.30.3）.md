...
containerd-1.7.19 + k8s-1.30.3（最新）（kubeadm方式）（containerd容器运行时版）

> kubeadm方式安装最新版k8s-1.30.3（containerd容器运行时）
>
> containerd-1.7.19 + k8s-1.30.3（最新）（kubeadm方式）
...
### 1、安装containerd-1.7.19（官方源、腾讯源）(rocky-9.4 )
...
```shell
yum -y install containerd.io-1.7.19
```
...
### 2、安装k8s（kubeadm-1.30.3、kubelet-1.30.3、kubectl-1.30.3）（官方源）(rocky-9.4 )
...
yum -y install kubeadm-1.30.3 kubelet-1.30.3 kubectl-1.30.3
...
kubeadm config images list --kubernetes-version=v1.30.3 --image-repository registry.aliyuncs.com/google_containers
```
...
kubeadm config images pull --kubernetes-version=v1.30.3 --image-repository registry.aliyuncs.com/google_containers
```
...
> 这里k8s-1.30.3，所以使用calico-v3.27.3版本（版本对应很关键）

```shell
# mkdir -p ~/calico-yml

# cd ~/calico-yml && wget https://github.com/projectcalico/calico/raw/v3.28.1/manifests/calico.yaml
```
