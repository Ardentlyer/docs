...
containerd-1.7.23 + k8s-1.32.3（最新）（kubeadm方式）（containerd容器运行时版）

> kubeadm方式安装最新版k8s-1.32.3（containerd容器运行时）
>
> containerd-1.7.23 + k8s-1.32.3（最新）（kubeadm方式）
...
### 2、安装k8s（kubeadm-1.32.3、kubelet-1.32.3、kubectl-1.32.3）（官方源）(rocky-9.5 )
