...
containerd-2.2.1 + k8s-v1.35.0（最新）（kubeadm方式）（containerd容器运行时版）

> kubeadm方式安装最新版k8s-v1.35.0（containerd容器运行时）
>
> containerd-2.2.1 + k8s-v1.35.0（最新）（kubeadm方式）
...
```shell
yum -y install containerd.io-2.2.1
```
...
[kubernetes]
name=Kubernetes
baseurl=https://mirrors.cloud.tencent.com/kubernetes_new/core:/stable:/v1.35/rpm/
enabled=1
gpgcheck=0
EOF
...
yum -y install kubeadm-1.35.0 kubelet-1.35.0 kubectl-1.35.0
...
certificateValidityPeriod: 87600h0m0s         # ✅ 普通证书有效期（10 年）
...
kubeadm config images list --kubernetes-version=v1.35.0 --image-repository registry.aliyuncs.com/google_containers
```
