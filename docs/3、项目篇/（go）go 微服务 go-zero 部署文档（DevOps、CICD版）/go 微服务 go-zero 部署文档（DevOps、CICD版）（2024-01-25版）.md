## go 微服务 go-zero 部署文档（DevOps、CICD版）（2024-01-25版）
...
>
> https://github.com/arklnk/ark-admin-zero（后端）
>
> https://github.com/arklnk/ark-admin-vuenext（前端）
...
- groups:
  ...
  token: 此处填写 kubeadm bootstrap token
  ttl: 24h0m0s
  usages:
  - signing
  - authentication
...
```shell
# 创建 ark_admin 数据库并且导入数据

mysql -h 192.168.1.200 -u root -P 30336 -pAdmin@2024 -e "create database \`ark_admin\`;"

mkdir -p ~/go-zero-rollout-yml

cd ~/go-zero-rollout-yml && wget https://github.com/arklnk/ark-admin-zero/raw/dev/build/deploy/sql/ark_admin.sql

kubectl -n mysql cp ark_admin.sql mysql-0:/
kubectl -n mysql exec -it mysql-0 bash

mysql -h 192.168.1.200 -P 30336 -pAdmin@2024 ark_admin < ark_admin.sql
```
...
cat > Dockerfile << 'EOF'
# 使用 alpine:3.19.0 基础镜像
FROM alpine:3.19.0

# 添加 libc6-compat 库
RUN apk add --no-cache libc6-compat
EOF
docker build -t ccr.ccs.tencentyun.com/huanghuanhui/alpine:3.19.0-libc6-compat .
docker push ccr.ccs.tencentyun.com/huanghuanhui/alpine:3.19.0-libc6-compat
```
...
GitRepo
代码仓库
http://gitlab.huanghuanhui.cloud/root/ark-admin-zero.git
...
Image
基础镜像
ccr.ccs.tencentyun.com/huanghuanhui/alpine:3.19.0-libc6-compat
```

```shell
#!/usr/bin/env groovy

def git_auth = "此处填写 Jenkins Git 凭据 ID"
def harbor_auth = "此处填写 Jenkins Harbor 凭据 ID"
def kubectl_auth = "此处填写 Jenkins Kubernetes 凭据 ID"
...
spec:
  ...
  - name: golang-cache
    persistentVolumeClaim:
      claimName: jenkins-prod-slave-golang-cache
  - name: go-build-cache
    persistentVolumeClaim:
      claimName: jenkins-prod-slave-go-build-cache
'''
        }
    }
            ...
            steps {
              container('golang') {
                sh """
                  ...
                  go mod tidy && go build -ldflags="-s -w" -o api app/core/cmd/api/core.go
                   """
                }
            }
        }
...
Log:
  ...
  Path: logs/rpc/admin-api   #日志存储路径，默认logs
  Level: error     #日志写入级别 info|error|severe
...
JwtAuth:
  AccessSecret: 此处填写服务访问密钥
  AccessExpire: 86400

Mysql:
  DataSource: root:Admin@2024@tcp(mysql-headless.mysql.svc.cluster.local:3306)/ark_admin?charset=utf8mb4&parseTime=true
...
ADD ./admin-api.yaml /usr/local/src/admin-api.yaml
CMD ["/usr/local/src/api", "-f", "/usr/local/src/admin-api.yaml"]
EOF
docker build -t ${HarborUrl}/go-zero/${AppName}:${GitBranch}-${GIT_COMMIT_MSG}-${BUILD_ID} . && docker images
'''
                }
            }
        }
        ...
        stage('推送镜像') {
            steps {
              container('docker') {
                ...
                docker push ${HarborUrl}/go-zero/${AppName}:${GitBranch}-${GIT_COMMIT_MSG}-${BUILD_ID}
                """
                   }
                }
            }
            ...
            steps {
              ...
              configFileProvider([configFile(fileId: "${kubectl_auth}", variable: 'kubeconfig')]) {
                ...
                mkdir -p ~/.kube && cp ${kubeconfig} ~/.kube/config
                 /app/kubectl-argo-rollouts-linux-amd64 set image ${AppName} "*=${HarborUrl}/go-zero/${AppName}:${GitBranch}-${GIT_COMMIT_MSG}-${BUILD_ID}" -n go-zero
                """
                   }
                }
            }
...
GitRepo
代码仓库
http://gitlab.huanghuanhui.cloud/root/ark-admin-vuenext.git
    ...
    stages {
        ...
        stage('代码编译') {
            steps {
              container('node') {
                sh """
                   npm install -g pnpm --registry=https://registry.npmmirror.com && pnpm --version && pnpm install --no-frozen-lockfile --registry=https://registry.npmmirror.com && pnpm build
                """
                }
            }
        }
...
http {
    ...
    server {
        ...
        location /api/{
            proxy_pass http://go-zero-api-svc:8001/;
...
EOF
cd 000 && docker build -t ${HarborUrl}/go-zero/${AppName}:${GitBranch}-${GIT_COMMIT_MSG}-${BUILD_ID} .
'''
                }
            }
        }
...
cd ~/go-zero-rollout-yml

kubectl create namespace go-zero
```

### 1、go-zero-api-rollout

```shell
cat > ~/go-zero-rollout-yml/go-zero-api-rollout.yml << 'EOF'
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: go-zero-api
        image: harbor.huanghuanhui.cloud/go-zero/go-zero-api:main-92ae07d-2
        ports:
        - name: http
          containerPort: 8001
          protocol: TCP
...
````shell
cat > go-zero-api-svc.yml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: go-zero-api-svc
  namespace: go-zero
  labels:
    app: go-zero-api
spec:
...
### 2、go-zero-vue-rollout

```shell
cat > ~/go-zero-rollout-yml/go-zero-vue-rollout.yml << 'EOF'
apiVersion: argoproj.io/v1alpha1
kind: Rollout
metadata:
  name: go-zero-vue
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: go-zero-vue
        image: harbor.huanghuanhui.cloud/go-zero/go-zero-vue:main-393cd95-1
        ports:
        - name: http
          containerPort: 80
          protocol: TCP
...
```shell
cat > go-zero-vue-svc.yml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: go-zero-vue-svc
  namespace: go-zero
  labels:
    app: go-zero-vue
spec:
...
```shell
cat > ~/go-zero-yml/go-zero-vue-Ingress.yml << 'EOF'
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: go-zero-vue-ingress
  namespace: go-zero
  annotations:
    nginx.ingress.kubernetes.io/ssl-redirect: 'true'
    nginx.ingress.kubernetes.io/proxy-body-size: '4G'
spec:
  ingressClassName: nginx
  rules:
  - host: go-zero-vue.huanghuanhui.cloud
    http:
      paths:
      - path: /
        pathType: Prefix
  ...
  tls:
  - hosts:
    - go-zero-vue.huanghuanhui.cloud
    secretName: go-zero-vue-ingress-tls
EOF
```

```shell
kubectl create secret -n go-zero \
tls go-zero-vue-ingress-tls \
--key=/root/ssl/huanghuanhui.cloud.key \
--cert=/root/ssl/huanghuanhui.cloud.crt
```

```shell
kubectl apply -f ~/go-zero-yml/go-zero-vue-Ingress.yml
```

> 访问地址：go-zero-vue.huanghuanhui.cloud
>
> 设置账号密码为：arklnk、123456

## 注：

> go-zero项目是一个微服务这里只部署了一个例子只有一个服务，比如rpc没上，部署其他的后端（pipeline基本上都是一样的）